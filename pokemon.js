...
module.exports = (sequelize, DataTypes) => { 
    return sequelize.define('Pokemon', {
      id: {
        ...
        primaryKey: true, //indique la clé primaire de la table (un pokemon doit avoir un id unique)
        autoIncrement: true 
      },
      name: {
      ...
      types: {
        ...
        validate: {
            isTypesValid(value) {
                if (!value) {
                    throw new Error("La propriété 'types' doit contenir au moins un type.")
                }
                if (value.split(',').length > 3) {
                    throw new Error("La propriété 'types' ne peut pas contenir plus de 3 types.")
                }
